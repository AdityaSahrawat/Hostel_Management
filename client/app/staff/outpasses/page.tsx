...
export default function OutpassesPage() {
  ...
  async function load() {
    ...
    setError(null);
    try {
      const data = await apiGet<OutPass[]>("/outpasses", auth.token);
      setItems(data);
    } catch (e) {
            ...
            <tbody>
              {items.map((o) => (
                <tr key={o.id} className="border-b border-foreground/5">
                  <td className="px-5 py-3 font-mono text-xs">{o.student_id}</td>
              ...
              {items.length === 0 && !loading ? (
                <tr>
                  <td className="px-5 py-6 text-sm opacity-70" colSpan={4}>
                    No outpasses.
                  </td>
                </tr>
              ) : null}
